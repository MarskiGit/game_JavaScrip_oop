...
class LevelSelect extends Common {
    ...
    createButton(value) {
        if (!userData.checkAvailabilityLevel(value)) return true;
    ...
    loadLevel(level) {
        ...
        if (!media.diamondsSprite) media.diamondsSprite = loader.loadImage('images/diamonds-transparent.png');
        if (!media.backgroundImage) media.backgroundImage = loader.loadImage('images/levelbackground.png');
        if (!media.swapSound) media.swapSound = loader.loadSound('sounds/stone_rock_or_wood_moved.mp3');
        if (!media.backgroundMusic) media.backgroundMusic = loader.loadSound('sounds/music-background.mp3');
